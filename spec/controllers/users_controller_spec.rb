		...
		describe "success" do
			...
			it "should deliver the confirmation email" do
				lambda do
				 ...
				 last_delivery.to.should include "john@sub.domain"
				 last_delivery.from.should include "no-reply@pomar.com"
				 last_delivery.subject.should include "Pomar! Account confirmation"
				 last_delivery.body.should include "/confirm?key=" + User.encrypt("john@sub.domain", User.find_by_username("john").sha1_salt)
			end
		end
	end
